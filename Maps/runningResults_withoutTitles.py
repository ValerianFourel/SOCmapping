...
PICTURE_VERSION = "SGT_Maps_MixedProcessing"
MAX_OC = 150
TIME_BEGINNING = 2007
TIME_END = 2023
INFERENCE_TIME = 2023
...
output_dir = "/home/valerian/SGTPublication/SOCmapping/SGT_Maps_MixedProcessing"
...
# Define SGT model paths with processing type
model_data = {
    "SGT_360k_RESCALED": {
        "display_name": "SGT 360k (Rescaled)",
        "type": "split",
        "processing": "rescaled",  # Apply rescaling
        ...
        "normalization_pkl": None
    },
    "SGT_1mil_RAW_split": {
        "display_name": "SGT 1mil (Raw Predictions - Split Files)",
        "type": "split",
        "processing": "raw",  # NO rescaling - direct mapping
    ...
    "SGT_1mil_RAW_single": {
        "display_name": "SGT 1mil (Raw Predictions - Single File)",
        "type": "single",
        "processing": "raw",  # NO rescaling - direct mapping
        "coordinates": "/home/valerian/SGTPublication/Weights-ResidualsModels-MappingInference-SOCmapping/Archive/finalResults2023_OC150_2007to2023_transform_normalize_loss_l1_runs_1_lr_0.0002_heads_8_layers_2_TemporalFusionTransformer_Inference/coordinates_1mil.npy",
        "predictions": "/home/valerian/SGTPublication/Weights-ResidualsModels-MappingInference-SOCmapping/Archive/finalResults2023_OC150_2007to2023_transform_normalize_loss_l1_runs_1_lr_0.0002_heads_8_layers_2_TemporalFusionTransformer_Inference/predictions_1mil.npy",
        "normalization_pkl": None
    }
}
...
def extract_normalization_params(pkl_path):
    ...
    try:
        ...
        # Extract actual targets from nested train_results and val_results
        train_targets = None
        val_targets = None
        ...
        if 'train_results' in data and isinstance(data['train_results'], dict):
            if 'targets' in data['train_results']:
                ...
                print(f"  ✓ Found training targets: shape {train_targets.shape}")
        ...
        if 'val_results' in data and isinstance(data['val_results'], dict):
            if 'targets' in data['val_results']:
                ...
                print(f"  ✓ Found validation targets: shape {val_targets.shape}")
        ...
        print(f"\n  ⚠ WARNING: Could not extract normalization parameters")
        return None, None
...
def rescale_predictions_to_target_distribution(predictions, target_mean, target_std, clip_min=2.5, clip_max=150):
    ...
    print(f"    Range before clip: [{rescaled.min():.2f}, {rescaled.max():.2f}]")
    ...
    print(f"      - Clipped to {clip_min}: {n_clipped_low:,} ({100*n_clipped_low/len(rescaled):.2f}%)")
    print(f"      - Clipped to {clip_max}: {n_clipped_high:,} ({100*n_clipped_high/len(rescaled):.2f}%)")
    
    return clipped

def load_and_validate_split_data(base_dir, file_pattern):
    """Load and concatenate split coordinate and prediction files"""
    coordinates_list = []
    predictions_list = []
    ...
    for coord_file, pred_file in file_pattern:
        ...
        if not os.path.exists(coord_path) or not os.path.exists(pred_path):
            print(f"  ⚠ WARNING: {coord_file} or {pred_file} not found - SKIPPING")
            continue
        ...
        print(f"  ✓ {coord_file}: {coords.shape[0]:,} points")
        
        if coords.shape[0] != preds.shape[0]:
            print(f"  ✗ ERROR: Shape mismatch! SKIPPING")
            continue
...
def create_sgt_maps():
    """Generate maps - 360k with rescaling, 1mil models with RAW predictions (NO TITLES)"""
    ...
    global_max = 150
    print(f"\n{'='*80}")
    print(f"FIXED COLOR SCALE: [{global_min}, {global_max}] SOC g/kg")
    print(f"{'='*80}")
    ...
    for model_key, model_info in model_data.items():
        try:
            ...
            print(f"Processing mode: {model_info['processing'].upper()}")
            print(f"{'='*80}")
            ...
            if model_info.get('type') == 'single':
                ...
                print(f"  ✓ Loaded: {coordinates.shape[0]:,} points")
            else:
            ...
            if model_info['processing'] == 'rescaled':
                print(f"\n  *** APPLYING RESCALING ***")
                ...
                # If no pickle, compute from data
                if mean_target is None:
                    # Fallback: use typical SOC distribution
                    mean_target = 22.527
                    std_target = 19.668
                    print(f"  Using fallback normalization: mean={mean_target:.3f}, std={std_target:.3f}")
                
                predictions = rescale_predictions_to_target_distribution(
                    predictions, mean_target, std_target, clip_min=2.5, clip_max=150
                )
            else:  # raw
                print(f"\n  *** NO RESCALING - USING RAW PREDICTIONS ***")
                print(f"  Clipping to [{global_min}, {global_max}] only")
                n_below = np.sum(predictions < global_min)
                n_above = np.sum(predictions > global_max)
                predictions = np.clip(predictions, global_min, global_max)
                print(f"    Clipped to {global_min}: {n_below:,} ({100*n_below/len(predictions):.2f}%)")
                print(f"    Clipped to {global_max}: {n_above:,} ({100*n_above/len(predictions):.2f}%)")
                print(f"    Final range: [{predictions.min():.2f}, {predictions.max():.2f}]")
            ...
            # Save
            filename = f"{PICTURE_VERSION}_{model_key}_{timestamp}.png"
...
if __name__ == "__main__":
    print("="*80)
    print("SGT MAPPING - MIXED PROCESSING")
    print("360k: Rescaled | 1mil models: RAW predictions (NO rescaling)")
    print("="*80)
    create_sgt_maps()
    print(f"\n{'='*80}")
