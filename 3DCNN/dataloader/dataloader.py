...
def normalize_batch(tensor_data):
    """
    Apply Min-Max scaling to each band independently for batched data.

    Args:
        tensor_data (torch.Tensor): Input tensor of shape (batch_size, num_years*4, H, W, bands)
    ...
    # Reshape to combine all dimensions except bands
    reshaped_tensor = tensor_data.view(-1, num_bands)

    # Calculate min and max for each band
    min_vals = reshaped_tensor.min(dim=0)[0]
    max_vals = reshaped_tensor.max(dim=0)[0]
    ...
    normalized_tensor = (reshaped_tensor - min_vals) / denominator

    # Reshape back to original dimensions
    return normalized_tensor.view(shape)
