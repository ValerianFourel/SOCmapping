...
def create_visualizations(subset_df, remaining_df, full_df, best_dist, best_params, dist_name, save_path, iteration=0):
    ...
    ax1.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, f'spatial_distribution_{timestamp}_iter{iteration}.png'), bbox_inches='tight')
    plt.close(fig1)
    ...
    if len(distances) > 0:
        ...
        ax2.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(save_path, f'min_distances_{timestamp}_iter{iteration}.png'), bbox_inches='tight')
        plt.close(fig2)
    ...
    ax3.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, f'oc_distribution_{timestamp}_iter{iteration}.png'), bbox_inches='tight')
    plt.close(fig3)
    ...
    ax4.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, f'kde_comparison_{timestamp}_iter{iteration}.png'), bbox_inches='tight')
    plt.close(fig4)

    # Create a combined plot for overview (optional)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, f'combined_visualization_{timestamp}_iter{iteration}.png'), bbox_inches='tight')
    plt.close()
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='ImagesOutput', help='Output directory')  # Changed to ImagesOutput
